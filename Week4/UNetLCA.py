...
class UpBlock(nn.Module):
    ...
    def forward(self, x, t, context):
        x = self.res(x, t)
        if isinstance(self.attn, AttentionBlock):
            x = self.attn(x, context)
        return x
...
class AttentionBlock(nn.Module):
    def __init__(self, n_channels: int, n_heads: int = 2, d_k: int = None, n_groups: int = 32, dropout: float = 0.1):
        ...
        if d_k is None:
            d_k = n_channels
        self.context_proj = nn.Linear(768, n_channels)
        self.norm = nn.GroupNorm(n_groups, n_channels)
        self.output = nn.Linear(n_heads * d_k, n_channels)
        self.dropout = nn.Dropout(dropout)
        self.attn = nn.MultiheadAttention(embed_dim=n_channels, num_heads=n_heads, dropout=dropout, batch_first=True)
    ...
    def forward(self, x: torch.Tensor, context):
        ...
        # Normalize
        x = self.norm(x)
        x = x.view(batch_size, n_channels, -1).permute(0, 2, 1)  # [B, HW, C] multiu head attn shape
...
class UNet(nn.Module):
    ...
    def forward(self, x: Tensor, t: Tensor, context: Tensor):
        B, C, H, W = x.shape
